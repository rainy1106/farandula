...
public class Communication {
    ...
    interface PresenterStateCheckCallback {
        void onStatus(PresenterStatus presenterStatus, StarPrinterStatus status);
    }
...
class SendCommandThread extends Thread {
    ...
    private static void resultSendCallback(final Communication.Result result, final int code, final Communication.SendCallback callback) {
        if (callback != null) {
...
class SendCommandForRedirectionThread extends Thread {
    ...
    @Override
    public void run() {
        List<Pair<String, Communication.CommunicationResult>> communicationResultList = new ArrayList<>();
        ...
        synchronized (mLock) {
            ...
            for (PrinterSettings printerSettings : mPrinterSettingsList) {
                ...
                communicationResultList.add(new Pair<>(portName, new Communication.CommunicationResult(result, code)));
    ...
    private static void resultSendCallback(final List<Pair<String, Communication.CommunicationResult>> communicationResultList, final Communication.PrintRedirectionCallback callback) {
        if (callback != null) {
            Handler handler = new Handler(Looper.getMainLooper());
            handler.post(new Runnable() {
                @Override
...
class SendCommandWithAutoInterfaceSelectThread extends Thread {
    ...
    private static void resultSendCallback(final Communication.Result result, final int code, final String portName, final Communication.AutoInterfaceSelectCallback callback) {
        if (callback != null) {
            Handler handler = new Handler(Looper.getMainLooper());
            handler.post(new Runnable() {
                @Override
                public void run() {
                    callback.onStatus(portName, new Communication.CommunicationResult(result, code));
                }
            });
        }
    }
...
class SendCommandWithAutoInterfaceSelectExtThread extends Thread {
    ...
    @Override
    public void run() {
        Communication.Result result = Result.ErrorOpenPort;
        int code = StarResultCode.FAILURE;
        String portName = null;
